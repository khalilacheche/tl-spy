...
class ControllerTracker:
    ...
    def _spawn_agents(self, sighting: Sighting) -> None:
            ...
            if agent.route_ahead:
                ...
                if r < 0.10:
                    ...
                    if lines:
                        ...
                        route = self.network.get_route_ahead(line, agent.current_stop, None)
                        if route:
                            exit_idx = self._pick_exit(route)
    ...
    def get_heatmap_points(self) -> list[tuple[float, float, float]]:
        """Return interpolated (lat, lon, weight) for all live agents."""
